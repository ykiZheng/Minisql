...
if __name__ == '__main__':
    index = Index()
    index_filepath = 'DBfiles/index.json'
    list_filepath = 'DBfiles/list.json'
    data_filepath = 'DBfiles/data.txt'
    ...
    select_res = index.Select_from_table(table_name, primary_key, 60, attribute, True)
    if select_res[0]:
        table_student = PrettyTable(select_column)
        temp = select_res[1][0:2]
        table_student.add_row(temp)
        print(table_student)
    else:
        print('Not Found')
